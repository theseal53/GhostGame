...
public class Kitchen : Room
{
	...
	int centerCounterMargin = 2;
	int pantryWall = 1;

	int kX, kY, kWidth, kHeight;
	int pX, pY, pWidth, pHeight;

	List<GameObject> pantryOptions;
	List<float> pantryChances;
	List<GameObject> pantryCornerOptions;
	...
	public Kitchen() : base()
	{
		...
		pantryOptions = new List<GameObject>()
		{
			tileSet.crate,
			tileSet.potatoSack,
			tileSet.flourSack,
		...
		pantryCornerOptions = new List<GameObject>()
		{
			tileSet.potatoSack,
			tileSet.flourSack,
			null
	...
	public override void SetupRoom(Board board, Doorway doorway)
	{
		base.SetupRoom(board, doorway);
	...
	public void GeneratePantryFurniture()
	{
		...
		GameObject prefab = RandomPantryItem(precompTotal);
		if (prefab != null)
		{
			GameObject cornerTarget = Object.Instantiate(prefab);
			cornerTarget.transform.position = new Vector2(pX, pY);
		}
		prefab = RandomPantryItem(precompTotal);
		if (prefab != null)
		{
			GameObject cornerTarget = Object.Instantiate(prefab);
			cornerTarget.transform.position = new Vector2(pX+pWidth-1, pY);
		}
		prefab = RandomPantryItem(precompTotal);
		if (prefab != null)
		{
			GameObject cornerTarget = Object.Instantiate(prefab);
			cornerTarget.transform.position = new Vector2(pX, pY+pHeight-1);
		}
		prefab = RandomPantryItem(precompTotal);
		if (prefab != null)
		{
			GameObject cornerTarget = Object.Instantiate(prefab);
			cornerTarget.transform.position = new Vector2(pX+pWidth-1, pY+pHeight-1);
		}
	...
	public void GenerateKitchenFurniture()
	{
		...
		GameObject fridge = Object.Instantiate(tileSet.fridge);
		GameObject oven = Object.Instantiate(tileSet.oven);
		float rng = Random.value;
		GameObject firstSpawn = rng > .5f ? fridge : oven;
		GameObject secondSpawn = rng > .5f ? oven : fridge;
		Rect firstSpawnRect;
		...
		firstSpawn.transform.position = new Vector2(firstSpawnRect.x, firstSpawnRect.y);
		secondSpawn.transform.position = new Vector2(secondSpawnRect.x, secondSpawnRect.y);
		AttemptObjectInstantiation(tileSet.plainCounter, firstCornerFiller);
		AttemptObjectInstantiation(tileSet.plainCounter, secondCornerFiller);
		...
		{
			GameObject target = Object.Instantiate(tileSet.cabinetCounter);
			target.transform.position = spot;
		}
	...
	public void GenerateCenterCounter()
	{
		...
		//Corners
		TileSetRegistry.I.floorTilemap.SetTile(new Vector3Int(cX, cY, 0), tileSet.counterBottomLeft);
		TileSetRegistry.I.floorTilemap.SetTile(new Vector3Int(cX + cWidth - 1, cY, 0), tileSet.counterBottomRight);
		TileSetRegistry.I.floorTilemap.SetTile(new Vector3Int(cX, cY + cHeight - 1, 0), tileSet.counterTopLeft);
		TileSetRegistry.I.floorTilemap.SetTile(new Vector3Int(cX + cWidth - 1, cY + cHeight - 1, 0), tileSet.counterTopRight);

		for (int xPos = cX + 1; xPos < cX + cWidth - 1; xPos++)
		{
			TileSetRegistry.I.floorTilemap.SetTile(new Vector3Int(xPos, cY, 0), tileSet.counterBottomCenter);
			TileSetRegistry.I.floorTilemap.SetTile(new Vector3Int(xPos, cY + cHeight - 1, 0), tileSet.counterTopCenter);
		}
		for (int yPos = cY + 1; yPos < cY + cHeight - 1; yPos++)
		{
			TileSetRegistry.I.floorTilemap.SetTile(new Vector3Int(cX, yPos, 0), tileSet.counterMidLeft);
			TileSetRegistry.I.floorTilemap.SetTile(new Vector3Int(cX + cWidth - 1, yPos, 0), tileSet.counterMidRight);
		}
		for (int xPos = cX + 1; xPos < cX + cWidth - 1; xPos++)
		{
			for (int yPos = cY + 1; yPos < cY + cHeight - 1; yPos++)
			{
				TileSetRegistry.I.floorTilemap.SetTile(new Vector3Int(xPos, yPos, 0), tileSet.counterMidCenter);
			}
		}

		GameObject gameObject = new GameObject("Counter Collider");
		gameObject.transform.position = new Vector2(cX + cWidth / 2f - .5f, cY + cHeight / 2f - .5f);
		BoxCollider2D collider = gameObject.AddComponent<BoxCollider2D>();
		collider.size = new Vector2(cWidth, cHeight);
	}

	public GameObject RandomPantryCornerItem(float precompTotal)
	{
		return WeightedChoice.Choose(pantryCornerOptions, pantryCornerChances, precompTotal);
	}

	public GameObject RandomPantryItem(float precompTotal)
	{
	...
	public override void PrintToTilesArray(short[,] tiles)
	{
		for (int yPos = kY; yPos < kY + kHeight; yPos++)
		{
			for (int xPos = kX; xPos < kX + kWidth; xPos++)
			{
				tiles[yPos,xPos] = (short)roomCode;
			}
		}
		...
		tiles[kitchenPantryMedium.y, kitchenPantryMedium.x] = (short)roomCode;
	}
}
